...
class PhysicalModel:
    ...
    def extended_propagation(self,object, verbose = False):
        """ 
        ...
        for p in range(1, self.n_patterns+1):
            Yi = self.propagate_object(object, p_no = p, verbose=verbose)
...
class dmd_patterns:
    ...
    """
    def __init__(self,ep_dx, ep_dy, dx, dy, nx, ny, device):
        self.ep_dx = max(round(ep_dx/dx),1)
        self.ep_dy = max(round(ep_dy/dy),1)
        self.nx, self.ny = nx, ny
        self.device = device

    def initialize_dmd(self):
        """
        Method: randomly initialize a pattern
        """
